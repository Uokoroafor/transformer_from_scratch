...
class Transformer(nn.Module):
    ...
    def get_src_mask(self, src: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            src (torch.Tensor): Source tensor of shape (batch_size, seq_len)
        Returns:
            torch.Tensor: Source mask tensor of shape (batch_size, 1, seq_len)
        """
        # A key-padding mask broadcasts correctly for both encoder self-attention
        # and decoder cross-attention.
        return (src != self.src_pad).unsqueeze(-2)
